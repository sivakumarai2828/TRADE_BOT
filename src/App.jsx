...
const POLL_MS = 3_000; // refresh UI every 3 seconds

export default function App() {
  const [data, setData] = useState(null);
  const [apiError, setApiError] = useState(false);
  const [lastUpdated, setLastUpdated] = useState(null);
  const [tick, setTick] = useState(0); // forces "X sec ago" to re-render
  ...
  // Tick every second so "X sec ago" updates live
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      <Sidebar exchangeName={data?.exchange_name} paperMode={data?.paper_mode} />
